...
bool MemoryFlash::begin(uint16_t sizeOfMemory)
{
    ...
    {
        readAll();
        // Serial.println("Initialization of flash succesful");
        return true;
    }
    else
    {
...
{
    EEPROM.get(0, flash);
    // Serial.println("All flash data read!");
}
...
bool MemoryFlash::storeDataToFlash(void)
{
    ...
    if (EEPROM.commit())
    {
        // Serial.println("Stored succesfully!");
        return true;
    }
    else
    {
        // Serial.println("failed to stored!!!");
        return false;
    }
}
...
bool MemoryFlash::setSSID(const char *newValue)
{
    ...
    else
    {
        ...
        {
            // Serial.println(String() + "SSID char length " + size + " exceeds the maximum limit " + (MAX_SSID_CHAR - 1));
        }
        else
        {
            // Serial.println(String() + "New data already in flash!");
        }
    }
    return false;
}
bool MemoryFlash::setPassword(const char *newValue)
{
    ...
    else
    {
        ...
        {
            // Serial.println(String() + "Password char length " + size + " exceeds the maximum limit " + (MAX_PASSWORD_CHAR - 1));
        }
        else
        {
            // Serial.println(String() + "New data already in flash!");
...
bool MemoryFlash::setMaximumBattery(float newValue)
{
    ...
    // Serial.println(String() + "before: " + a + " after: " + b);
    ...
    if (battVoltageNew != flash.batteryMaximum)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set : " + (flash.batteryMaximum >> 8) + "." + (flash.batteryMaximum & 0x00FF) + "V");
        return true;
    }
    else
    {
...
bool MemoryFlash::setMinimumBattery(float newValue)
{
    ...
    if (battVoltageNew != flash.batteryMinimum)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set : " + (flash.batteryMinimum >> 8) + "." + (flash.batteryMinimum & 0x00FF) + "V");
        return true;
    }
    else
    {
...
bool MemoryFlash::setBaudrateSerial(bool type, uint8_t newValue)
{
    if (newValue != flash.baudrateSerial[type])
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set : " + baudrate[newValue]);
        return true;
    }
    else
    {
...
bool MemoryFlash::setTimeSchedulerDatalog(bool scheduleType, uint8_t loggerType, uint8_t index, uint16_t newValue)
{
    ...
    if (newValue != oldValue)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set (HH:MM): " + (newValue / 60) + ":" + (newValue % 60));
        return true;
    }
    else
    {
...
bool MemoryFlash::setDateSchedulerDatalog(bool scheduleType, uint8_t loggerType, uint8_t newDate, uint8_t newMonth, uint8_t newYear)
{
    ...
    if (newValue != oldValue)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set :" + newDate + "/" + newMonth + "/" + newYear);
        return true;
    }
    else
    {
...
bool MemoryFlash::setEnableDateScheduler(bool scheduleType, uint8_t loggerType, bool newValue)
{
    ...
    if (newValue != oldValue)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "new data " + newValue + " set at index " + index);
        return true;
    }
    else
    {
...
bool MemoryFlash::setDimScreenTimer(uint8_t newValue)
{
    if (newValue != flash.dimScreenTimer)
    {
        ...
        storeDataToFlash();
        // Serial.println(String() + "New data set : " + dim[newValue]);
        return true;
    }
    else
    {
