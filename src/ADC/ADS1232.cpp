...
float ADS1232::getWeightInUnit(byte channel)
{
    ...
    // //Serial.println(String() + "State Point Channel " + channel + ": " + adcState);
    ...
    case 1:
        weight = (weightCalculationInGram((fdata->getAdcCalibrationPoint(channel, (adcState - 1))), 0.00,
                                          (fdata->getAdcCalibrationPoint(channel, adcState)), fdata->getGramCalibrationPoint(channel, (adcState - 1)), adcValue));
        break;

    default:
        weight = (weightCalculationInGram((fdata->getAdcCalibrationPoint(channel, (adcState - 1))), fdata->getGramCalibrationPoint(channel, (adcState - 2)),
                                          (fdata->getAdcCalibrationPoint(channel, adcState)), fdata->getGramCalibrationPoint(channel, (adcState - 1)), adcValue));
        break;
    }
...
{
    int pointCalibrationStatus = getPointDoneCalibrated(fdata->getPointCalibrationStatus(channel));
    pointCalibrationStatus = constrain(pointCalibrationStatus - 1, 0, MAX_POINT_CAL);
    // //Serial.println(String() + "Max Point Calib Channel " + channel + ": " + pointCalibrationStatus + " points");

    for (byte i = 0; i < pointCalibrationStatus; i++)
    {
        if (adcValue < fdata->getAdcCalibrationPoint(channel, i) + ADC_OFFSET)
            return i;
    }
